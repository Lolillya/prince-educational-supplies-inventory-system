  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="ghost"
          className={`group flex h-12 w-12 items-center justify-center rounded-xl bg-transparent transition-all duration-300 hover:bg-emerald-200/60 ${className}`}
        >
          <Banknote
            className="!h-5 !w-5 text-slate-500 transition-colors duration-300 group-hover:text-emerald-500"
            strokeWidth={2.5}
          />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="shadow-none" popoverTarget="">
        <p className="font-medium text-slate-700">New Payment</p>
        ...
        <div className="mt-4 flex flex-col gap-2">
          ...
          <div>
            <div className="flex gap-2">
              ...
              <div>
                <Label
                  className={`${calculatedChange > 0 ? "text-emerald-500" : "text-slate-400"}`}
                >
                  Change
                </Label>
                <Input
                  value={calculatedChange.toFixed(2)}
                  className={`w-full shadow-none focus:outline focus:outline-2 focus:outline-slate-200 ${
                    calculatedChange > 0
