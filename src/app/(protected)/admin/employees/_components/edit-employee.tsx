...
const EditEmployeeState = ({ id }: { id: string }) => {
  ...
  const deleteEmployee = api.employees.delete.useMutation({
    onSuccess: () => {
  ...
  const validateForm = (): EmployeeFormErrors => {
    ...
    if (!firstName) {
      ...
      newErrors.firstName =
        "First Name must only contain letters and be at least 2 characters long.";
    ...
    if (contact && !/^\d{9,15}$/.test(contact)) {
      newErrors.contact =
        "Contact must be numeric and between 9-15 digits long.";
    }
    ...
    if (addressLine && addressLine.trim().length < 5) {
      ...
      newErrors.addressLine =
        "Address Line must be at most 100 characters long.";
    }
