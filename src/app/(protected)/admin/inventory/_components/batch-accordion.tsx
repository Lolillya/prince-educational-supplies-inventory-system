                    ...
                    batch.supplierUnits.map((supplierUnit, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          ...
                          supplierUnit.ConversionRate.length > 0 ? (
                            supplierUnit.ConversionRate.filter(
                              (conversionRate) =>
                                conversionRate.fromUnit.name ===
                                supplierUnit.unit.name,
                            ).map((conversionRate, rateIndex) => (
                              <div key={rateIndex}>
                                &gt; {conversionRate.conversion_rate}{" "}
                                {conversionRate.toUnit.name}
                              </div>
                            ))
                          ) : (
                        ...
                        <TableCell>
                          P {supplierUnit.price?.toFixed(2) || "N/A"}
                        </TableCell>
                        <TableCell>
