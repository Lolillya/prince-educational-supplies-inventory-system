//NEW CODE FOR EDIT BATCH BUT NOT WORKING
// "use client";
...
import InventoryCard, {
  InventoryCardRef,
} from "../../_components/inventory-card";
import { api } from "~/trpc/react";
...
const EditBatch = () => {
  ...
  useEffect(() => {
    if (inventoryDataResponse && inventoryIdAsNumber) {
      ...
      if (matchedVariant) {
        ...
        // Step 3: Check if this variant_id exists in the BatchVariant table
        const matchedBatchVariant = matchedVariant.BatchVariant.find(
          (batchVariant) => {
            console.log("Checking batch variant:", batchVariant);
            console.log("Batch ID in BatchVariant:", batchVariant.batch_id);
            ...
            if (batchVariant.variant_id === matchedVariant.variant_id) {
              console.log(
                "Found matching batch variant for variant_id:",
                batchVariant,
              );
              return true;
            }
  ...
  const getChangeStatus = (
    current: StockUnitData,
    original: StockUnitData,
    index: number,
  ) => {
    // New unit (exists in current but not in original)
    if (!original && current) return "new";
  ...
  // In EditBatch component
  const { mutateAsync: updateSupplierUnits } =
    api.inventory.updateSupplierUnits.useMutation();
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      setIsSaving(true);
      const updates = Object.values(inventoryCardRefs.current).map(
        async (cardRef) => {
          const cardData = cardRef.getStockData();
          ...
          // Get supplier ID from first existing unit or original data
          const baseSupplierId =
            cardData.originalUnits[0]?.supplierId ||
          ...
          const unitsToUpdate = cardData.currentUnits.map(
            (currentUnit, index) => {
              const originalUnit = cardData.originalUnits[index];
              const status = getChangeStatus(currentUnit, originalUnit, index);
              ...
              if (
                currentUnit.conversionQty &&
                (isNaN(currentUnit.conversionQty) ||
                  currentUnit.conversionQty < 0)
              ) {
                throw new Error(
                  "Conversion quantity must be a positive number.",
                );
              }
              if (
                (currentUnit.conversionQty && !currentUnit.conversionUnit) ||
                ...
                throw new Error(
                  "Both conversion quantity and conversion unit must be provided.",
                );
              }
              if (
                currentUnit.conversionUnit &&
                !/^[a-zA-Z\s]+$/.test(currentUnit.conversionUnit)
              ) {
                throw new Error(
                  "Conversion unit must be selected from the list and contain only letters.",
                );
              }
              ...
              return {
                ...
                data: {
                  ...
                  unit: currentUnit.unit,
                  conversionQty: currentUnit.conversionQty
                    ? currentUnit.conversionQty.toString()
                    : "",
                  conversionUnit: currentUnit.conversionUnit || "",
                  supplierId:
                    currentUnit.supplierId ||
                    originalUnit?.supplierId ||
                    baseSupplierId,
                },
              };
            },
          ...
          // Handle units removed from the end
          const removedCount =
            cardData.originalUnits.length - cardData.currentUnits.length;
          if (removedCount > 0) {
            const removedUnits = cardData.originalUnits
              .slice(-removedCount)
              .map((unit) => ({
  ...
  return (
    <section className="flex h-screen w-screen flex-col gap-3 overflow-hidden p-10 pb-0">
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <span className="font-bold">
          {itemData?.item?.name || "N/A"} - {itemData?.name || "N/A"} -{" "}
          ...
          itemData.BatchVariant.map((batchVariant) => {
            ...
            return (
              <InventoryCard
                ...
                ref={(el) => {
                  if (el) {
                    inventoryCardRefs.current[batchVariant.batch_variant_id] =
                      el;
                  }
                }}
